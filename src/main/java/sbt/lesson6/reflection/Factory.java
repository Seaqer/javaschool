...
public class Factory implements Factorable {
    ...
    public static Factory createNew(Class<?> cls) throws ClassNotFoundException, InstantiationException, IllegalAccessException, IOException, InvocationTargetException {
    ...
    public <T> T getBean(Class<T> cls) {
        ...
        if (candidates.isEmpty()) {
            throw new InvalidStateException("There is not candidate!");
        }
        if (candidates.size() > 1) {
            throw new InvalidStateException("There are more than one candidate: " + candidates.toString());
        }
        ...
        final List<Class<?>> classes = getAllClassesByPredicate(baseFile, c -> {
            if (c.isAnnotationPresent(Component.class)) {
                if (Modifier.isInterface(c.getModifiers()) || Modifier.isAbstract(c.getModifiers())) {
                    throw new IllegalArgumentException(c.toString() + " should not be abstract or interface");
                } else {
                    return true;
                }
            } else {
    ...
    private Class<?> findBeanClsFor(Class<?> d, Map<Class<?>, List<Class<?>>> beansTypes) {
        ...
        if (result.size() > 1) {
            throw new InvalidStateException("More than one candidate have been found!");
        }
        if (result.isEmpty()) {
            throw new InvalidStateException("Could not find dependency: " + d.toString());
        }
        return result.get(0);
    }
    ...
    private void invokeAnnotationMethods(Class<?> beanCls, Object bean, Class<? extends Annotation> annotationClass) throws InvocationTargetException, IllegalAccessException {
